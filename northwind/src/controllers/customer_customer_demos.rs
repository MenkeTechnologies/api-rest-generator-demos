...
use crate::models::_entities::customer_customer_demos::{ActiveModel, Entity as CustomerCustomerDemos, Model};
...
async fn load_item(ctx: &AppContext, id: i32) -> Result<Model> {
    CustomerCustomerDemos::find_by_id(id)
        .one(&ctx.db)
        .await?
...
pub async fn list(State(ctx): State<AppContext>) -> Result<Response> {
    format::json(CustomerCustomerDemos::find().all(&ctx.db).await?)
}
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/customer_customer_demos")
        .add("/", get(list))
        .add("/", post(add))
        .add("/{id}", get(get_one))
