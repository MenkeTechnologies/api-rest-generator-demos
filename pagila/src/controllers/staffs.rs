...
use crate::models::_entities::staffs::{ActiveModel, Entity as Staffs, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
        ...
        item.username = sea_orm::ActiveValue::set(self.username.clone());
        item.password = sea_orm::ActiveValue::set(self.password.clone());
        item.last_update = sea_orm::ActiveValue::set(self.last_update.clone());
    }
}

async fn load_item(ctx: &AppContext, id: i32) -> Result<Model> {
    Staffs::find_by_id(id)
        .one(&ctx.db)
        .await?
...
pub async fn list(State(ctx): State<AppContext>) -> Result<Response> {
    format::json(Staffs::find().all(&ctx.db).await?)
}
...
pub async fn add(State(ctx): State<AppContext>, Json(params): Json<Params>) -> Result<Response> {
        let item = ActiveModel {
            ...
            username: sea_orm::ActiveValue::set(params.username.clone()),
            password: sea_orm::ActiveValue::set(params.password.clone()),
